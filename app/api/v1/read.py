...
async def get_active_style_profiles(db: AsyncSession = Depends(get_session)):
...
async def get_styles_by_type(type: str, db: AsyncSession = Depends(get_session)):
...
async def list_style_profiles(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_session)):
...
async def get_style_profile(id: uuid.UUID, db: AsyncSession = Depends(get_session)):
